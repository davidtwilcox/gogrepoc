...
loggingHandler = logging.handlers.RotatingFileHandler('gogrepo.log', mode='a+', maxBytes=1024 * 1024 * LOG_MAX_MB,
                                                      backupCount=LOG_BACKUPS, encoding=None, delay=True)
...
HTTP_RETRY_DELAY = 5  # in seconds #If you reduce this such that the wait between the first and third try is less
# than 10 seconds, you're gonna have a bad time with the 503 error.
HTTP_RETRY_COUNT = 3
HTTP_TIMEOUT = 60

HTTP_GAME_DOWNLOADER_THREADS = 4
HTTP_PERM_ERRORCODES = (404,
                        403)  # 503 was in here GOG uses it as a request to wait for a bit when it's under stress.
# The time out appears to be ~10 seconds in such cases.
...
SKIP_MD5_FILE_EXT = ['.txt', '.zip',
                     '']  # Removed tar.gz as it can have md5s and is actually parsed as .gz so wasn't working
for i in range(1, 21):
    n = i
    a = "." + f'{n:03}'
    ...
    try:
        if data is not None:
            if byte_range is not None:
                response = session.post(url, params=args, headers={'Range': 'bytes=%d-%d' % byte_range},
                                        timeout=HTTP_TIMEOUT, stream=stream, data=data)
            else:
        ...
        response.raise_for_status()
            ...
            requests.HTTPError, requests.URLRequired, requests.Timeout, requests.ConnectionError,
        ...
        if _retry:
            ...
            return request(session=session, url=url, args=args, byte_range=byte_range, retries=retries - 1,
    ...
    with token_lock:
        ...
        try:
            if time_now + 300 > session.token['expiry']:
                ...
                try:
                    token_response = session.get(GOG_TOKEN_URL, params={'client_id': '46899977096215655',
                    ...
                    if retries > 0:
                        ...
                        sys.exit(1)
                    if _retry:
                        warn('token refresh failed: %s (%d retries left) -- will retry in %ds...' % (
...
def path_preserving_split_ext(path):
    path_without_extensions = os.path.join(os.path.dirname(path), os.path.basename(path).rsplit(os.extsep, 1)[
        0])  # Need to improve this to handle eg tar.gz
    ...
    for newDownload in newitem.downloads + newitem.galaxyDownloads + newitem.sharedDownloads:
            ...
            if oldDownload.md5 is not None:
                if oldDownload.md5 == newDownload.md5 and oldDownload.size == newDownload.size and oldDownload.lang == newDownload.lang:
                    if oldDownload.name == newDownload.name:
                        ...
                        break  # Can't be overridden so end it now
                    if oldDownload.name != newDownload.name and candidate is None:  # Will not override and gets
                        # overridden by a perfect match (also allows only one match)
        ...
        if candidate is not None:
            ...
            if candidate.name != newDownload.name:
                info('  -> in folder_name "{}" a download has changed name "{}" -> "{}"'.format(newitem.folder_name,
                                                                                                candidate.name,
                                                                                                newDownload.name))
    ...
    for newExtra in newitem.extras:
        ...
        if candidate is not None:
            ...
            if candidate.name != newExtra.name:
                info('  -> in folder_name "{}" an extra has changed name "{}" -> "{}"'.format(newitem.folder_name,
                                                                                              candidate.name,
                                                                                              newExtra.name))
...
def fetch_chunk_tree(response, session):
    ...
    if file_ext not in SKIP_MD5_FILE_EXT:
        try:
            ...
            warn("unexpected connection error fetching md5 data for {}".format(
                chunk_url) + " This error may be temporary. Please retry in 24 hours.")
            debug("The handled exception was:")
            if rootLogger.isEnabledFor(logging.DEBUG):
                log_exception('')
            debug("End exception report.")
            ...
            warn("unexpected content decoding error fetching md5 data for {}".format(
                chunk_url) + " This error may be temporary. Please retry in 24 hours.")
            debug("The handled exception was:")
            if rootLogger.isEnabledFor(logging.DEBUG):
                log_exception('')
    ...
    if fetch_md5:
        ...
        if file_ext not in SKIP_MD5_FILE_EXT:
            try:
                ...
                warn("unexpected connection error fetching md5 data for {}".format(
                    d.name) + " This error may be temporary. Please retry in 24 hours.")
                debug("The handled exception was:")
                if rootLogger.isEnabledFor(logging.DEBUG):
                    log_exception('')
                debug("End exception report.")
            ...
            for os_type in downloads_dict[lang]:
                if os_type in os_list:
                    for download in downloads_dict[lang][os_type]:
                        ...
                        for href in hrefs:
                            if not (unreleased or (file_info_success and md5_success)):
                                ...
                                for key in download:
                                    try:
                                        ...
                                        if tmp_contents != download[key]:
                                            debug(
                                                "GOG Data Key, %s , for download clashes with Download Data Key "
                                                "storing detailed info in secondary dict" % key)
                                ...
                                if d.gog_data.size == "0 MB":  # Not Available
                                    ...
                                    if d.md5_exempt is True or d.md5 is None:
                                        md5_success = True
                        ...
                        elif file_info_success and md5_success:  # Will be the current d because no more are created
                            ...
                            for href_d in href_ds:
                                if not any_file_info_success:
                                    if href_d[1]:
                                        ...
                                        warn("Successfully fetched file info from %s but no md5 data was available" %
                                             href_d[0].href)
                            if not any_file_info_success:
                ...
                for key in extra:
                    try:
                        ...
                        if tmp_contents != extra[key]:
                            debug(
                                "GOG Data Key, %s , for extra clashes with Extra Data Key storing detailed info in "
                                "secondary dict" % key)
        ...
        if update_item.name is not None:
            ...
            if deDuplicatedName is not None:
                if update_item.name != deDuplicatedName:
                    info(
                        '  -> ' + update_item.name + 'already exists in this game entry with a different size and/or '
                                                     'md5, this file renamed to ' + deDuplicatedName)
                    update_item.name = deDuplicatedName
                deDuplicatedList.append(update_item)
            else:
                info(
                    '  -> ' + update_item.name + 'already exists in this game entry with same size/md5, skipping '
                                                 'adding this file to the manifest')
        else:
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if (not strictDupe) and (None in md5list or potentialItem.md5 is None):
                    return None
                else:
...
def process_argv(argv):
    ...
    g1.add_argument('-resumemode', action="store", choices=['noresume', 'resume', 'onlyresume'], default='resume',
                    help="how to handle resuming if necessary")
    g1.add_argument('-strictverify', action="store_true", help="clear previously verified unless md5 match")
    g1.add_argument('-strictdupe', action="store_true",
                    help="missing MD5s do not default to checking only file size (missing MD5s only match other "
                         "missing MD5s rather than any value)")
    ...
    g1.add_argument('-installers', action='store', choices=['standalone', 'both'], default='standalone',
                    help='GOG Installer type to use: standalone or both galaxy and standalone. Default: standalone ('
                         'Deprecated)')
    ...
    g5.add_argument('-ids', action='store', help='id(s)/titles(s) of (a) specific game(s) to update', nargs='*',
                    default=[])
    g5.add_argument('-skipids', action='store', help='id(s)/titles(s) of (a) specific game(s) not to update', nargs='*',
                    default=[])
    ...
    g1.add_argument('-skipshared', action='store_true',
                    help='skip downloading installers shared between Galaxy and standalone (Deprecated)')
    g2 = g1.add_mutually_exclusive_group()
    g2.add_argument('-skipextras', action='store_true', help='skip downloading of any GOG extra files')
    g2.add_argument('-skipgames', action='store_true',
                    help='skip downloading of any GOG game files (deprecated, use -skipgalaxy -skipstandalone -skipshared instead)')
    g3 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g3.add_argument('-ids', action='store', help='id(s) or title(s) of the game in the manifest to download', nargs='*',
                    default=[])
    g3.add_argument('-skipids', action='store', help='id(s) or title(s) of the game(s) in the manifest to NOT download',
                    nargs='*', default=[])
    ...
    g1.add_argument('-skipfiles', action='store', help='file name (or glob patterns) to NOT download', nargs='*',
                    default=[])
    g1.add_argument('-wait', action='store', type=float,
                    help='wait this long in hours before starting', default=0.0)  # sleep in hr
    ...
    g4 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g4.add_argument('-skipos', action='store', help='skip downloading game files for operating system(s)', nargs='*',
                    default=[])
    g4.add_argument('-os', action=storeExtend, help='download game files only for operating system(s)', nargs='*',
                    default=[])
    g5 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g5.add_argument('-lang', action=storeExtend, help='download game files only for language(s)', nargs='*', default=[])
    g5.add_argument('-skiplang', action='store', help='skip downloading game files for language(s)', nargs='*',
                    default=[])
    g1.add_argument('-nolog', action='store_true', help='doesn\'t writes log file gogrepo.log')
    g1.add_argument('-debug', action='store_true', help="Includes debug messages")
    ...
    g2.add_argument('-skipos', action='store', help='skip importing game files for operating system(s)', nargs='*',
                    default=[])
    g2.add_argument('-os', action='store', help='import game files only for operating system(s)', nargs='*', default=[])
    g3 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g3.add_argument('-skiplang', action='store', help='skip importing game files for language(s)', nargs='*',
                    default=[])
    ...
    g1.add_argument('-skipshared', action='store_true',
                    help='skip downloading installers shared between Galaxy and standalone')
    g5 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g5.add_argument('-ids', action='store', help='id(s) or title(s) of the game in the manifest to import', nargs='*',
                    default=[])
    g5.add_argument('-skipids', action='store', help='id(s) or title(s) of the game(s) in the manifest to NOT import',
                    nargs='*', default=[])
    ...
    g5 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g5.add_argument('-ids', action='store', help='id(s) or title(s) of the game in the manifest to backup', nargs='*',
                    default=[])
    g5.add_argument('-skipids', action='store', help='id(s) or title(s) of the game(s) in the manifest to NOT backup',
                    nargs='*', default=[])
    g2 = g1.add_mutually_exclusive_group()  # below are mutually exclusive        
    g2.add_argument('-skipos', action='store', help='skip backup of game files for operating system(s)', nargs='*',
                    default=[])
    g2.add_argument('-os', action='store', help='backup game files only for operating system(s)', nargs='*', default=[])
    g3 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g3.add_argument('-skiplang', action='store', help='skip backup of game files for language(s)', nargs='*',
                    default=[])
    ...
    g1.add_argument('-skipshared', action='store_true',
                    help='skip backup of any installers included in both the GOG Galalaxy and Standalone sets')
    g1.add_argument('-nolog', action='store_true', help='doesn\'t writes log file gogrepo.log')
    g1.add_argument('-debug', action='store_true', help="Includes debug messages")
    ...
    g1.add_argument('-forceverify', action='store_true',
                    help='also verify files that are unchanged (by gogrepo) since they were last successfully verified')
    ...
    g3 = g1.add_mutually_exclusive_group()  # below are mutually exclusive
    g3.add_argument('-ids', action='store', help='id(s) or title(s) of the game in the manifest to verify', nargs='*',
                    default=[])
    g3.add_argument('-skipids', action='store', help='id(s) or title(s) of the game[s] in the manifest to NOT verify',
                    nargs='*', default=[])
    g3.add_argument('-id', action='store', help='(deprecated) id or title of the game in the manifest to verify')
    g1.add_argument('-skipfiles', action='store', help='file name (or glob patterns) to NOT verify', nargs='*',
                    default=[])
    g4 = g1.add_mutually_exclusive_group()  # below are mutually exclusive        
    g4.add_argument('-skipos', action='store', help='skip verification of game files for operating system(s)',
                    nargs='*', default=[])
    g4.add_argument('-os', action='store', help='verify game files only for operating system(s)', nargs='*', default=[])
    g5 = g1.add_mutually_exclusive_group()  # below are mutually exclusive    
    g5.add_argument('-skiplang', action='store', help='skip verification of game files for language(s)', nargs='*',
                    default=[])
    ...
    g1.add_argument('-skipstandalone', action='store_true',
                    help='skip verification of any GOG standalone installer files')
    g1.add_argument('-skipshared', action='store_true',
                    help='skip verification of any installers included in both the GOG Galalaxy and Standalone sets')
    g1.add_argument('-nolog', action='store_true', help='doesn\'t writes log file gogrepo.log')
    g1.add_argument('-debug', action='store_true', help="Includes debug messages")
    ...
    g1 = sp1.add_parser('clear_partial_downloads',
                        help='Permanently remove all partially downloaded files from your game directory')
...
def cmd_login(user, passwd):
    ...
    page_response = request(loginSession, GOG_AUTH_URL, args={'client_id': GOG_CLIENT_ID,
    ...
    if (not loginForm) or len(loginForm.findall('.//div[@class="g-recaptcha form__recaptcha"]')) > 0:
        if not loginForm:
            error(
                "Could not locate login form on login page to test for reCAPTCHA, please contact the maintainer. In "
                "the meantime use a browser (Firefox recommended) to sign in at the below url and then copy & paste "
                "the full URL")
        else:
            error(
                "gog is asking for a reCAPTCHA :(  Please use a browser (Firefox recommended) to sign in at the below "
                "url and then copy & paste the full URL")
    ...
    if not token_data['login_code']:
        ...
        if token_data['two_step_url'] is not None:
            ...
            page_response = request(loginSession, token_data['two_step_url'],
                                    data={'second_step_authentication[token][letter_1]':
                                              token_data['two_step_security_code'][0],
                                          'second_step_authentication[token][letter_2]':
                                              token_data['two_step_security_code'][1],
                                          'second_step_authentication[token][letter_3]':
                                              token_data['two_step_security_code'][2],
                                          'second_step_authentication[token][letter_4]':
                                              token_data['two_step_security_code'][3],
    ...
    if token_data['login_code']:
        ...
        token_response = request(loginSession, GOG_TOKEN_URL, args={'client_id': '46899977096215655',
                                                                    'client_secret': '9d85c43b1482497dbbce61f6e4aa173a433796eeae2ca8c5f6129f2dc4de46d9',
                                                                    'grant_type': 'authorization_code',
                                                                    'code': token_data['login_code'],
                                                                    'redirect_uri': 'https://embed.gog.com'
    ...
    if not loginSession:
        ...
        try:
            gogSession.headers = {'User-Agent': USER_AGENT,
                                  'Authorization': 'Bearer ' + gogSession.token['access_token']}
...
def cmd_update(os_list, lang_list, skipknown, updateonly, partial, ids, skipids, skipHidden, installers, resumemode,
               strict, strictDupe):
    ...
    if needresume:
        ...
        while not done:
            ...
            data_response = request(updateSession, api_url,
                                    args={'mediaType': media_type, 'sortBy': 'title', 'page': str(i)})
            ...
            for item_json_data in json_data['products']:
                ...
                for key in item_json_data:
                    try:
                        ...
                        if tmp_contents != item_json_data[key]:
                            debug(
                                "GOG Data Key, %s , for item clashes with Item Data Key storing detailed info in "
                                "secondary dict" % key)
                ...
                if not done:
                    if item.title not in skipids and str(item.id) not in skipids:
                        ...
                        if (not partial) or (updateonly and item.has_updates) or (
                                skipknown and item.id not in known_ids):
                            items.append(item)
                    else:
    ...
    resumeprop = {'os_list': os_list, 'lang_list': lang_list, 'installers': installers, 'strict': strict,
                  'complete': False, 'skipknown': skipknown, 'partial': partial, 'updateonly': updateonly,
                  'strictDupe': strictDupe}
    ...
    for item in sorted(items, key=lambda item: item.title):
        ...
        try:
            ...
            for key in item_json_data:
                if key not in ["downloads", "extras", "galaxyDownloads",
                               "dlcs"]:  # DLCS lose some info in processing, need to fix that when extending.  #This
                    # data is going to be stored after filtering (#Consider storing languages / OSes in
                    # case new ones are added)
                    try:
                        tmp_contents = item[key]
                        if tmp_contents != item_json_data[key]:
                            debug(
                                "Detailed GOG Data Key, %s , for item clashes with Item Data Key attempting to store "
                                "detailed info in secondary dict" % key)
                            try:
                                tmp_contents = item.gog_data[key]
                                if tmp_contents != item_json_data[key]:
                                    debug(
                                        "GOG Data Key, %s  ,for item clashes with Item Secondary Data Key storing "
                                        "detailed info in tertiary dict" % key)
            ...
            # Indepent Deduplication to make sure there are no doubles within galaxyDownloads or downloads to avoid
            # weird stuff with the comprehention.
    ...
    if needresume:
        ...
        if resumemode != 'onlyresume':
            ...
            cmd_update(save_os_list, save_lang_list, save_skipknown, save_updateonly, save_partial, ids, skipids,
                       skipHidden, save_installers, resumemode, save_strict, save_strictDupe)
...
def cmd_import(src_dir, dest_dir, os_list, lang_list, skipextras, skipids, ids, skipgalaxy, skipstandalone, skipshared):
    ...
    for (root, dirnames, filenames) in os.walk(src_dir):
        for f in filenames:
            if (os.extsep + f.rsplit(os.extsep, 1)[
                1]).lower() not in SKIP_MD5_FILE_EXT:  # Need to extend this to cover tar.gz too
...
def cmd_download(savedir, skipextras, skipids, dryrun, ids, os_list, lang_list, skipgalaxy, skipstandalone, skipshared,
                 skipfiles, covers, backgrounds, downloadLimit=None):
    sizes, rates, errors = {}, {}, {}
    ...
    if not items:
        if ids and skipids:
            error('no game(s) with id(s) in "{}" was found'.format(
                ids) + 'after skipping game(s) with id(s) in "{}".'.format(skipids))
        elif ids:
        ...
        for cur_dir in sorted(os.listdir(downloadingdir)):
            ...
            if os.path.isdir(cur_fulldir):
                if cur_dir not in all_items_by_title:
                    ...
                    expected_filenames = []
                    for game_item in all_items_by_title[cur_dir].downloads + all_items_by_title[
                        cur_dir].galaxyDownloads + all_items_by_title[cur_dir].sharedDownloads + all_items_by_title[
                                         cur_dir].extras:
                    ...
                    for cur_dir_file in os.listdir(cur_fulldir):
                        if os.path.isdir(os.path.join(downloadingdir, cur_dir, cur_dir_file)):
                            ...
                            if not dryrun:
                                shutil.rmtree(os.path.join(downloadingdir, cur_dir,
                                                           cur_dir_file))  # There shouldn't be subdirectories here
                                # ?? Nuke to keep clean.
                        else:
    ...
    for item in sorted(items, key=lambda g: g.folder_name):
            ...
            with ConditionalWriter(os.path.join(item_homedir, INFO_FILENAME)) as fd_info:
                ...
                if hasattr(item, 'gog_messages') and item.gog_messages:
                    ...
                    for gog_msg in item.gog_messages:
                        fd_info.write(
                            u'{0}{1}{0}'.format(os.linesep, html2text(gog_msg).strip().replace("\n", os.linesep)))
            ...
            if not os.path.exists(key_local_path_file):
                if os.path.exists(images_key_dir_name):
                    try:
                        ...
                        error(
                            "Could not delete potential old image files, aborting update attempt. Please make sure "
                            "folder and files are writeable and that nothing is accessing the !image folder")
                        raise
                os.makedirs(images_key_dir_name)
        ...
        for game_item in item.downloads + item.galaxyDownloads + item.sharedDownloads + item.extras:
            ...
            if downloadLimit is not None and ((sum(sizes.values()) + game_item.size) > downloadLimit):
                info('     skip       %s (size %s would exceed download limit (%s/%s) )' % (
                    game_item.name, megs(game_item.size), megs(sum(sizes.values())), megs(downloadLimit)))
                continue
            ...
            try:
                    ...
                    if os.path.exists(path):
                        ...
                        if file_sz < sz:  # preallocate extra space
                            if platform.system() == "Windows":
                                try:
                                    ...
                                    preH = ctypes.windll.kernel32.CreateFileW(compat_downloading_path,
                                ...
                                if sys.version_info[0] >= 4 or (sys.version_info[0] == 3 and sys.version_info[1] >= 3):
                                    info("increasing preallocation to '%d' bytes for '%s' using posix_fallocate " % (
                                        sz, downloading_path))
                        ...
                        if os.path.exists(downloading_path):
                            ...
                            if file_sz < sz:  # preallocate extra space
                                if platform.system() == "Windows":
                                    try:
                                        preH = -1
                                        info(
                                            "increasing preallocation to '%d' bytes for '%s' " % (sz, downloading_path))
                                        preH = ctypes.windll.kernel32.CreateFileW(compat_downloading_path,
                                                                                  GENERIC_READ | GENERIC_WRITE, 0, None,
                                                                                  OPEN_EXISTING, 0, None)
                                        if preH == -1:
                                    ...
                                    if sys.version_info[0] >= 4 or (
                                            sys.version_info[0] == 3 and sys.version_info[1] >= 3):
                                        info(
                                            "increasing preallocation to '%d' bytes for '%s' using posix_fallocate " % (
                                                sz, downloading_path))
                                        with open(downloading_path, "r+b") as f:
                                            try:
                                                os.posix_fallocate(f.fileno(), 0, sz)
                            ...
                            if platform.system() == "Windows":
                                ...
                                if sys.version_info[0] >= 4 or (sys.version_info[0] == 3 and sys.version_info[1] >= 3):
                                    info("attempting preallocating '%d' bytes for '%s' using posix_fallocate " % (
                                        sz, downloading_path))
                ...
                if chunk_tree is not None:
                    ...
                    if expected_size != sz:
                        with lock:
                            error(
                                "XML verification data size does not match manifest size for %s. manifest %d, "
                                "received %d, skipping."
                                    ...
                                    with open_notruncwrrd(downloading_path) as out:
                                        ...
                                        if valid:
                                            ...
                                            while not downloadSegmentSuccess and retries >= 0:
                                                try:
                                                    response = request(downloadSession, href, byte_range=(start, end),
                                                                       stream=True)
                                                    ...
                                                    if hdr != '%d-%d/%d' % (start, end, sz):
                                                        ...
                                                        if dlsz == (end - start) + 1 and out.tell() == end + 1:
                                                            ...
                                                            if retries > 0:
                                                                warn(
                                                                    "failed to download %s, byte_range=%s (%d retries "
                                                                    "left) -- will retry in %ds..." % (
                                                                        os.path.basename(path), str(se), retries,
                                                                        HTTP_RETRY_DELAY))
                                                            else:
                                ...
                                if hdr != '%d-%d/%d' % (start, end, sz):
                                    ...
                                    if dlsz == (end - start) + 1 and out.tell() == end + 1:
                                        ...
                                        with lock:
                                            ...
                                            if retries > 0:
                                                warn(
                                                    "failed to download %s, byte_range=%s (%d retries left) -- will "
                                                    "retry in %ds..." % (
                ...
                if succeed and sizes[path] == 0:
                    ...
                    with lock:
                        info("not moving uncompleted download '%s', success: %s remaining bytes: %d / %d " % (
                            downloading_path, str(succeed), sizes[path], sz))
            except IOError as e:
                with lock:
                    warn("The handled exception was:")
                    log_exception('')
...
def cmd_verify(gamedir, skipextras, skipids, check_md5, check_filesize, check_zips, delete_on_fail, clean_on_fail, ids,
               os_list, lang_list, skipgalaxy, skipstandalone, skipshared, skipfiles, force_verify):
    ...
    if skipids:
        ...
        games_to_check = [game for game in games_to_check_base if
                          (game.title not in skipids and str(game.id) not in skipids)]
    ...
    for cur_dir in sorted(os.listdir(cleandir)):
        ...
        if os.path.isdir(cur_fulldir) and cur_dir not in ORPHAN_DIR_EXCLUDE_LIST:
            if cur_dir not in items_by_title:
                ...
                # dir is valid game folder, check its files
                expected_filenames = []
                for game_item in items_by_title[cur_dir].downloads + items_by_title[cur_dir].galaxyDownloads + \
                                 items_by_title[cur_dir].sharedDownloads + items_by_title[cur_dir].extras:
                    expected_filenames.append(game_item.name)
                for cur_dir_file in os.listdir(cur_fulldir):
...
def update_self():
    ...
    response = gitSession.get(REPO_HOME_URL + NEW_RELEASE_URL, stream="False", timeout=HTTP_TIMEOUT,
                              headers={'If-Modified-Since': 'Mon, 16 Jul 2018 08:51:22 GMT'})
    response.raise_for_status()
...
def main(args):
    ...
    if args.command == 'login':
        ...
        cmd_update(args.os, args.lang, args.skipknown, args.updateonly, not args.full, args.ids, args.skipids,
                   args.skiphidden, args.installers, args.resumemode, args.strictverify, args.strictdupe)
        ...
        cmd_download(args.savedir, args.skipextras, args.skipids, args.dryrun, args.ids, args.os, args.lang,
                     args.skipgalaxy, args.skipstandalone, args.skipshared, args.skipfiles, args.covers,
                     args.backgrounds, args.downloadlimit)
    elif args.command == 'import':
        # Hardcode these as false since extras currently do not have MD5s as such skipgames would give nothing and
        # skipextras would change nothing. The logic path and arguments are present in case this changes,
        # though commented out in the case of arguments)
        ...
        if args.skipgames:
            ...
            args.skipgalaxy = True
            args.skipshared = True
        cmd_import(args.src_dir, args.dest_dir, args.os, args.lang, args.skipextras, args.skipids, args.ids,
                   args.skipgalaxy, args.skipstandalone, args.skipshared)
        ...
        cmd_verify(args.gamedir, args.skipextras, args.skipids, check_md5, check_filesize, check_zips, args.delete,
                   args.clean, args.ids, args.os, args.lang, args.skipgalaxy, args.skipstandalone, args.skipshared,
                   args.skipfiles, args.forceverify)
        ...
        if args.skipgames:
            ...
            args.skipgalaxy = True
            args.skipshared = True
        cmd_backup(args.src_dir, args.dest_dir, args.skipextras, args.os, args.lang, args.ids, args.skipids,
                   args.skipgalaxy, args.skipstandalone, args.skipshared)
...
class Wakelock:
    ...
    def __init__(self):
        ...
        if platform.system() == "Windows":
            ...
            # Windows is not particularly consistent on what is required for a wakelock for a script that often uses
            # a USB device, so define WAKELOCK for easy changing. This works on Windows 10 as of the October 2017
            # update.
        ...
        if platform.system() == "Darwin":
            ...
            self.libIOKit.IOPMAssertionCreateWithName.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.c_void_p,
    ...
    def _CFSTR(py_string):
        return CoreFoundation.CFStringCreateWithCString(None, py_string.encode('utf-8'),
    ...
    def _get_inhibitor(self):
        try:
            return DBusSystemInhibitor('org.freedesktop.login1', '/org/freedesktop/login1',
    ...
    def take_wakelock(self):
        ...
        if platform.system() == "Darwin":
            ...
            if self._PMassertID.value == 0:
                self._PMerrcode, self._PMassertID = self._IOPMAssertionCreateWithName(a, self._kIOPMAssertionLevelOn,
                                                                                      "gogrepoc")
                self._PMassertion = a
...
class DBusSystemInhibitor:
    ...
    def __init__(self, name, path, interface, method=["Inhibit"]):
        ...
        serviceIntrospection = xml.etree.ElementTree.fromstring(
            PyQt5.QtDBus.QDBusReply(introspection.call("Introspect")).value())
        methodExists = False
    ...
    def uninhibit(self):
        if self.cookie is not None:
            pass  # It's not possible to release this file handle in QtDBus (since the QDUnixFileDescriptor is a
            # copy). The file handle is automatically released when the program exits.
...
class GnomeSessionInhibitor(DBusSessionInhibitor):
    ...
    def inhibit(self):
        if self.cookie is None:
            self.cookie = PyQt5.QtDbus.QDBusReply(
                self.iface.call(self.methods[0], self.APPNAME, GnomeSessionInhibitor.TOPLEVEL_XID, self.REASON),
                GnomeSessionInhibitor.INHIBIT_SUSPEND).value()
